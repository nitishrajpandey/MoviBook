...
import { fetchExploreApi } from "../../store/explorePageSlice/explorePageSlice";
...
function ExploreMovies() {
  ...
  const page = useSelector((state) => state.explore?.pageNo);
  const totalPage = useSelector((state) => state.explore?.totalPageNo);
  const loding = useSelector((state) => state.explore?.loding);
  ...
  useEffect(() => {
    dispatch(fetchExploreApi("/discover/movie"));
  }, []);

  const maindata = useSelector((state) => state.explore?.exploreMoviesData);
  console.log(maindata);

  // &&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&  unlimated loding

  ("/discover/movie?include_adult=true&include_video=false&language=en-US&page=2&sort_by=popularity.desc'");
  ...
  const handleInfiniteScroll = () => {
    if (!loding && page < totalPage) {
      ...
      if (scrollPosition >= pageHeight - pageHeight / 2) {
        dispatch(
          fetchExploreApi(
            `/discover/movie?include_adult=true&include_video=false&language=en-US&page=${
              page + 1
